...
export async function GET(req: NextRequest) {
    ...
    if (typeof folder === "string") {
        ...
        return NextResponse.json({ error: 'Invalid folder paramter'}, { status: 400 });
    }
    ...
    if (typeof countParam === "string") {
        ...
        if (isNaN(count)) {
            return NextResponse.json({ error: 'Invalid count paramter' }, { status: 400});
        }
    } else {
        count = 8;
    }
    ...
    try {
        ...
        return NextResponse.json({ error: 'Error reading image directory' }, { status: 500 });
    }
